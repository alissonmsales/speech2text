...
parser.add_argument('--mode', type=str, default="train",
                    help='mode: train/test/test_on_train')
...
parser.add_argument('--save_every', type=int, default=50000,
                    help='save state every x iteration')
...
parser.add_argument('--rnn_num_units', type=int, default=500,
                    help='number of hidden units if the network is RNN')
parser.add_argument('--equal_split', type=bool, default=True,
                    help='use trainEqual.csv and valEqual.csv')
...
if (args.equal_split):
    ...
    train_listfile = open(
        "/mnt/hdd615/Hrayr/Spoken-language-identification/trainingDataNew.csv", "r")
    test_listfile = open(
        "/mnt/hdd615/Hrayr/Spoken-language-identification/valDataNew.csv", "r")
...
if args.mode == 'train':
    ...
    for epoch in range(start_epoch, args.epochs):
        ...
        state_name = \
            '/home/alissonsales/PycharmProjects/speech2text/theano/networks/' \
            'states/%s.epoch%d.test%.5f.state' % \
            (network_name, epoch, test_loss)
